    ...
    def action_import(self):
        ...
        for sh in sheets:
            for row in range(2, sh.nrows):
                ...
                if not product_uom:
                    _logger.info(u'第%s行缺少单位' % row_n)
                    raise osv.except_osv(_(u'警告'), _(u'第%s行缺少单位') % row_n)
                else:
                    if product_uom in uom_dict:
                        data['uom_id'] = uom_dict[product_uom]
                        data['uom_po_id'] = uom_dict[product_uom]
                    else:
                        _logger.info(u'系统中缺少名字为%s的单位' % product_uom)
                        raise osv.except_osv(_(u'警告'), _(u'系统中缺少名字为%s的单位') % product_uom)
                if is_pos:
                    if is_pos not in ('1','0'):
                        _logger.info(u'第%s行可用于POS数值非法' % row_n)
                        raise osv.except_osv(_(u'警告'), _(u'第%s行可用于POS数值非法') % row_n)
                    else:
                        if is_pos == '1':
                ...
                data['barcode'] = ean_13
                if not name:
                    _logger.info(u'第%s行产品名称未填写' % row_n)
                    raise osv.except_osv(_(u'警告'), _(u'第%s行产品名称未填写') % row_n)
                data['name'] = name
                if sale_ok in ('Y', 'y'):
                ...
                if not type:
                    _logger.info(u'第%s行产品类型名称未填写' % row_n)
                    raise osv.except_osv(_(u'警告'), _(u'第%s行产品类型名称未填写') % row_n)
                data['type'] = type
                if default_code:
                ...
                if list_price:
                    if isinstance(list_price, float) or isinstance(list_price, int):
                        ...
                        _logger.info(u'第%s行标价填写错误' % row_n)
                        raise osv.except_osv(_(u'警告'),_(u'第%s行标价填写错误') % row_n)
                if standard_price:
                    if isinstance(standard_price, float) or isinstance(standard_price, int):
                        data['standard_price'] = standard_price
                    else:
                        _logger.info(u'第%s行成本价填写错误' % row_n)
                        raise osv.except_osv(_(u'警告'), _(u'第%s行成本价填写错误') % row_n)
                if company_name:
                    company_id = company_dict.get(company_name, False)
                    if not company_id:
                        _logger.info(u'第%s行公司填写错误' % row_n)
                        raise osv.except_osv(_(u'警告'), _(u'第%s行公司填写错误') % row_n)
                    company_id_l = company_id
                    data['company_id'] = company_id_l
                if state:
                    if state not in ('draft', 'sellable', 'end', 'obsolete'):
                        _logger.info(u'第%s行状态填写错误' % row_n)
                        raise osv.except_osv(_(u'警告'),_(u'第%s行状态填写错误') % row_n)
                    data['state'] = state
                if manager_name:
                    product_manager = user_dict.get((manager_name, company_id_l), False)
                    if not product_manager:
                        _logger.info(u'第%s行产品经理填写错误' % row_n)
                        raise osv.except_osv(_(u'警告'), _(u'第%s行产品经理填写错误') % row_n)
                ...
                if warranty:
                    if isinstance(warranty, float) or isinstance(warranty, int):
                        ...
                        _logger.info(u'第%s行质保填写错误' % row_n)
                        raise osv.except_osv(_(u'警告'), _(u'第%s行质保填写错误') % row_n)
                if sale_delay:
                    if isinstance(sale_delay, float) or isinstance(sale_delay, int):
                        data['sale_delay'] = sale_delay
                    else:
                        _logger.info(u'第%s行客户提前交货周期填写错误' % row_n)
                        raise osv.except_osv(_(u'警告'), _(u'第%s行客户提前交货周期填写错误') % row_n)
                if volume:
                    if isinstance(volume, float) or isinstance(volume, int):
                        data['volume'] = volume
                    else:
                        _logger.info(u'第%s行体积填写错误' % row_n)
                        raise osv.except_osv(_(u'警告'), _(u'第%s行体积填写错误') % row_n)
                if weight:
                    if isinstance(weight, float) or isinstance(weight, int):
                        data['weight'] = weight
                    else:
                        _logger.info(u'第%s行毛重填写错误' % row_n)
                        raise osv.except_osv(_(u'警告'), _(u'第%s行毛重填写错误') % row_n)
                if weight_net:
                    if isinstance(weight_net, float) or isinstance(weight_net, int):
                        data['weight_net'] = weight_net
                    else:
                        _logger.info(u'第%s行净重填写错误' % row_n)
                        raise osv.except_osv(_(u'警告'), _(u'第%s行净重填写错误') % row_n)
                if category:
                    if category.strip() not in category_dict:
                        _logger.info(u'第%s行内部分类填写错误' % row_n)
                        raise osv.except_osv(_(u'警告'), _(u'第%s行内部分类填写错误') % row_n)
                ...
                if public_category:
                    ...
                    for pca in public_category.split("|"):
                        if pca.strip() not in category_public_dict:
                            _logger.info(u'第%s行公开的产品分类填写错误' % row_n)
                            raise osv.except_osv(_(u'警告'),_(u'第%s行公开的产品分类填写错误') % row_n)
                ...
                if pos_category:
                    if pos_category.strip() not in pos_category_dict:
                        _logger.info(u'第%s行公开的pos类别填写错误' % row_n)
                        raise osv.except_osv(_(u'警告'), _(u'第%s行公开的pos类别填写错误') % row_n)
                ...
                if partner:
                    for par in partner.split("|"):
                        ...
                        if not partner_id_d:
                            _logger.info(u'第%s行供应商填写错误' % row_n)
                            raise osv.except_osv(_(u'警告'), _(u'第%s行供应商填写错误') % row_n)
                        data2 = {
                            'name': partner_id_d,
        ...
        if return_list:
            ...
            _logger.info(u'导入出错')
            raise osv.except_osv(_(u'警告'), _(u'导入出错'))
