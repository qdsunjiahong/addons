    ...
    def copy_product_company(self, cr, uid):
        ...
        for line in template_obj.browse(cr, uid, template_ids):
            ...
            for company_id in company_ids:
                template_obj.copy(cr, uid, line.id,
                                  {'name': name, 'default_code': default_code, 'company_id': company_id})
        return True
...
class qdodoo_res_company(models.Model):
    _inherit = 'res.company'
    """
    公司增加辅助核算项
    """
    analytic_account_id = fields.Many2one('account.analytic.account', string=u'辅助核算想', required=True)
