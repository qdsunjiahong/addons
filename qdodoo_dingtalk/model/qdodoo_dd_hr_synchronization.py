...
class qdodoo_department_auto(models.Model):
    ...
    def department_create_item2(self, key, department_name_list, access_token, department_name_id,
        ...
        :param key: 部门名称
        :param department_name_list: dict{部门名称：部门ID}
        :param access_token: 钉钉接口AccessToken
        :param department_name_id: dict{部门名称：上级部门ID}
        :param erp_department_parentid: dict{部门名称:上级部门名称}
        :return:
        """
        value = erp_department_parentid.get(key, '')
        if value in department_name_list:  #上级部门存在
            if key not in department_name_list:  #但本部门不存在
                #在钉钉中创建本部门
                parent_id = department_name_id.get(value)  #上级部门ID
                data = {
