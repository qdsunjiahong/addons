...
class qdodoo_warehouse_ratesearch(osv.Model):
    ...
    def action_open_new_window(self, cr, uid, ids, context=None):
        ...
        print '1111111111111',datetime.datetime.now()
        cr.execute('select id from stock_location ')
        for location in cr.fetchall():
            # 从份中获取指定库位的数据
            sale_num = {}
            cr.execute("select product_id from stock_quant  where location_id=%s"% location )
            for quant in cr.fetchall():
                print '1111111111111111111',quant
                cr.execute("SELECT * FROM stock_move where create_date >= '%s'  and  create_date<='%s' and  location_dest_id=%s and product_id=%s and location_id=%s " % (datetime.datetime.strptime(search_obj.from_date,'%Y-%m-%d %H:%M:%S'),datetime.datetime.strptime(search_obj.to_date,'%Y-%m-%d %H:%M:%S'),location_cus_ids,quant,location))
                stock_move_obj=cr.fetchall()
                ...
                for stock_id_record in stock_move_obj:
        ...
        # 遍历本地的库位
        print '1111111111111',datetime.datetime.now()
        print '222222222222222',datetime.datetime.now()
        ...
        print '222222222222222',datetime.datetime.now()
        print '333333333333333',datetime.datetime.now()
                ...
                if balance_ids:
                    ...
                    turnrate_id = turnrate_obj.create(cr, uid,
                                                      {'product': product_id, 'ware_name': location_id,
                                                       ...
                                                       'number': product_sale_num,
                                                       'invoice_state': search_obj.invoice_state})
        print '333333333333333333',datetime.datetime.now()
