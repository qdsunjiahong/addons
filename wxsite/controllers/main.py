...
class website_sale(http.Controller):
    def user_check(self):
        ...
        if not request.session.has_key('user_id'):  #通过session信息判断是否登陆，修改为odoo登陆的session键
            wx_appid = 'wxde2cf9dbf3c35624'  #微信服务号APPID
            wx_redirect = 'http://www.qdodoo.com/wxsite/wxoauth'  #微信授权后的回调地址，不可包含GET参数
    ...
    def wx_oauth(self, **post):
        ...
        """
        wx_appid = 'wxde2cf9dbf3c35624'  #微信服务号APPID
        wx_appsecret = '88beb863cce598171eb5f8f11c2ec9ec'  #微信服务号APPSecret
    ...
    @http.route(['/shop/wx/lunch',
                 '/shop/wx/lunch/<model("product.public.category"):category>',], type='http', auth="public", website=True)
    ...
    @http.route(['/shop/wx/wxpay'], type='http', auth="public", website=True)
    ...
    @http.route('/shop/wx/backnotify', type='http', auth='none', methods=['POST'])
    ...
    def get_didi(self, **post):
        ...
        values['nonceStr'] = str(time.time())
        return request.website.render("wxsite.didi",values)
