...
class qdodoo_purchase_price_wizard(models.Model):
    ...
    search_choice = fields.Selection(((1, u'年度'), (2, u'季度'), (3, u'月份'), (4, u'日期'), (5, u'时间段')),
    ...
    period_id = fields.Many2one('account.period', string=u'会计期间')
    ...
    def action_done(self):
        ...
        if self.company_id.name != u'惠美集团':
            if self.search_choice == 1 and self.year:
                start_datetime = self.year.name + "-01-01"
                end_datetime = self.year.name + "-12-12"
                if self.product_id:
                    sql = """
                        select
            ...
            elif self.search_choice == 2 and self.year and self.quarter:
                if self.quarter == 1:
                    start_datetime = self.year + "-01-01"
                    end_datetime = self.year + '-03-31'
                elif self.quarter == 2:
                    start_datetime = self.year + '-04-01'
                    end_datetime = self.year + '-06-30'
                elif self.quarter == 3:
                    start_datetime = self.year + '-07-01'
                    end_datetime = self.year + '-09-30'
                else:
                    start_datetime = self.year + '-10-01'
                    end_datetime = self.year + '12-31'
                if self.product_id:
                    sql = """
                        select
                            ail.product_id as product_id,
                    ...
                    sql = """
                        ...
                        group by ail.product_id,ail.quantity,ail.price_unit,spt.default_location_dest_id,ai.id,ail.invoice_id,pir.invoice_id,po.id,pir.purchase_id,po.picking_type_id,spt.id,po.state,po.company_id,po.date_order
                        """ % (start_datetime, end_datetime, self.company_id.id)
            elif self.search_choice == 3 and self.period_id:
                start_datetime = self.period_id.date_start
                end_datetime = self.product_id.date_stop
                if self.product_id:
                    sql = """
                        select
                            ail.product_id as product_id,
                    ...
                    select
                        ...
                        group by ail.product_id,ail.quantity,ail.price_unit,spt.default_location_dest_id,ai.id,ail.invoice_id,pir.invoice_id,po.id,pir.purchase_id,po.picking_type_id,spt.id,po.state,po.company_id,po.date_order
                        """ % (start_datetime, end_datetime, self.company_id.id)
            elif self.search_choice == 4 and self.date:
                if self.product_id:
                    sql = """
                        select
                            ail.product_id as product_id,
                        ...
                        from account_invoice_line ail
                            ...
                            LEFT JOIN purchase_order po ON po.id = pir.purchase_id
                            LEFT JOIN stock_picking_type spt ON po.picking_type_id = spt.id
                        where po.state = 'done' and ai.state != 'cancel' and po.date_order = '%s' and and po.company_id = %s
                        group by ail.product_id,ail.quantity,ail.price_unit,spt.default_location_dest_id,ai.id,ail.invoice_id,pir.invoice_id,po.id,pir.purchase_id,po.picking_type_id,spt.id,po.state,po.company_id,po.date_order
                        """ % (self.product_id.id, self.date, self.company_id.id)
                else:
                    sql = """
                    select
                            ail.product_id as product_id,
            ...
            elif self.search_choice == 5 and self.start_date and self.end_date:
                ...
                raise except_orm(_(u'警告'), _(u'请检查您的查询条件1'))
        else:
            if self.search_choice == 1 and self.year:
                start_datetime = self.year.name + "-01-01"
                end_datetime = self.year.name + "-12-12"
                ...
                raise except_orm(_(u'警告'), _(u'请检查您的查询条件2'))
