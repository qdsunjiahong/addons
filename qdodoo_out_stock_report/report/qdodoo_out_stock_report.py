    ...
    def init(self, cr):
        ...
        cr.execute("""
            create or replace view qdodoo_out_stock_report as (
                ...
                where sm.state='done' and (sl.usage='transit' or sl.id=%s) and sm.location_id != %s
                group by
                    sm.id, sm.location_id, sp.partner_id, sm.product_id,sp.date_done,sp.id,
                    sm.price_unit, sm.product_uom_qty, sm.product_uom
        )"""%(location_cus_ids,location_ids))
