...
class qdodoo_stock_in_analytic_wizard(models.Model):
    ...
    def action_done(self):
        ...
        un_ids.unlink()
        supplier_model, supplier_id = self.env['ir.model.data'].get_object_reference('stock',
        ...
        sql_select_base = """
            ...
            ai.payment_term as payment_term_id,
        ...
        if int(self.search_choice) == 1:
            ...
            sql_l1 = sql1_select_base + sql_select_base + sql_from_base + sql1_from_base + sql_where_base
            sql_l2 = "select" + sql_select_base + sql_from_base + sql2_from_base + sql_where_base + " and ai.type = 'in_refund'"
            ...
            if res:
                for i in res:
                    ...
                    # (供应商，产品，编码，单位，公司,年份)
                    k = (i[2], i[3], i[4], i[8], i[10], year_l)
                    if k in product_list:
                        product_num_dict[k] += i[5]
                        product_amount_dict[k] += i[7]
                    else:
                        product_num_dict[k] = i[5]
                        product_amount_dict[k] = i[7]
            ...
            if res2:
                for i2 in res2:
                    ...
                    if i2[8]:
                        data2['property_supplier_payment_term'] = i2[8]
                    cre_obj3 = report_obj.create(data2)
                    result_list.append(cre_obj3.id)
                    # (供应商，产品，编码，单位，公司,年份)
                    k2 = (i2[1], i2[2], i2[3], i2[7], i2[9], year_l2)
                    if k2 in product_list:
                        product_num_dict[k2] -= i2[4]
                        product_amount_dict[k2] -= i2[6]
                    else:
                        product_num_dict[k2] = -i2[4]
                        product_amount_dict[k2] = -i2[6]
        ...
        #####月份查询
            ...
            for per_ids_l in per_list:
                ...
                sql_l1 = sql1_select_base + sql_select_base + sql_from_base + sql1_from_base + sql_where_base2
                sql_l2 = "select" + sql_select_base + sql_from_base + sql2_from_base + sql_where_base2 + " and ai.type = 'in_refund'"
                ...
                if res:
                    for i in res:
                        ...
                        result_list.append(cre_obj1.id)
                        # (供应商，产品，编码，单位，公司，月份)
                        k = (i[2], i[3], i[4], i[8], i[10], month_l)
                        if k in product_list:
                            product_num_dict[k] += i[5]
                            product_amount_dict[k] += i[7]
                        else:
                ...
                if res2:
                    for i2 in res2:
                        ...
                        result_list.append(cre_obj3.id)
                        # (供应商，产品，编码，单位，公司，月份)
                        k2 = (i2[1], i2[2], i2[3], i2[7], i2[9], month_l)
                        if k2 in product_list:
                            product_num_dict[k2] -= i2[4]
                            product_amount_dict[k2] -= i2[6]
                        else:
