...
class qdodoo_search_stock_in(models.Model):
    ...
    def action_done(self):
        ...
        if self.date_start and not self.date_end:
            ...
            purchase_ids = purchase_obj.search([('date_order', '<=', datetime_end), ('state', '=', 'done')])
            ...
            purchase_ids = purchase_obj.search([('state', '=', 'done')])
        ...
        view_model, view_id = self.env['ir.model.data'].get_object_reference('qdodoo_stock_in_report', 'qdodoo_stock_in_report_tree')
        return {
            'name': _('工资核算'),
            'view_type': 'form',
            "view_mode": 'tree',
