...
class qdoo_invoice_report(osv.osv):
    _name = "qdoo.invoice.report"
    _description = "销售及退货"
    _auto = False
    _rec_name = 'date'
    ...
    def _compute_amounts_in_user_currency(self, cr, uid, ids, field_names, args, context=None):
        ...
        res = {}
        ctx = context.copy()
        for item in self.browse(cr, 1, ids, context=context):

            #compute cost price
            cost_price = 0
            stock = ''
            invoice_line = invoice_line_obj.browse(cr, 1, item.id, context=context)
            if invoice_line.origin:
                picking_name = invoice_line.origin.split(':')[0]

                picking_id = stock_picking_obj.search(cr, 1, [('name','=',picking_name),('company_id','=',item.company_id.id)],context=context)
                stock_move = stock_picking_obj.browse(cr, 1, picking_id, context=context).move_lines
                for move_line in stock_move:
                    if move_line.product_id == item.product_id:
                        cost_price= move_line.price_unit * move_line.product_uom_qty
                        if item.type == 'out_invoice':
                            stock = move_line.location_id.location_id.name
                        if item.type == 'out_refund':
                            stock = move_line.location_dest_id.location_id.name
    ...
    _columns = {
        ...
        'price_unit':fields.float(u'单价', readonly=True),
        'uom_name': fields.char('Reference Unit of Measure', size=128, readonly=True),
        'product_type': fields.char(u'产品类型', readonly=True),
        ...
        'categ_name': fields.char('Category of Product', readonly=True),
        ...
        'product_cost': fields.function(_compute_amounts_in_user_currency, string=u"成本额", type='float', digits_compute=dp.get_precision('Account'), multi="_compute_amounts"),
        'stock': fields.function(_compute_amounts_in_user_currency, string=u"物流中心", type='char', multi="_compute_amounts"),
    }
    _order = 'date desc'
    ...
    _depends = {
        'account.invoice': [
            ...
            'period_id', 'residual', 'state', 'type', 'user_id', 'section_id',
        ],
        'account.invoice.line': [
            'account_id', 'invoice_id', 'price_subtotal', 'product_id',
            'quantity', 'uos_id', 'price_unit',
        ],
    ...
    def _select(self):
        select_str = """
            SELECT sub.id, sub.date, sub.product_id, sub.price_unit, sub.partner_id, sub.country_id,
                sub.payment_term, sub.period_id, sub.uom_name, sub.product_type, sub.section_id, sub.currency_id, sub.journal_id,
                sub.fiscal_position, sub.user_id, sub.company_id, sub.nbr, sub.type, sub.state,
                sub.categ_id, sub.categ_name, sub.date_due, sub.account_id, sub.account_line_id, sub.partner_bank_id,
    ...
    def _sub_select(self):
        select_str = """
                SELECT min(ail.id) AS id,
                    ...
                    ail.product_id, ail.price_unit, ai.partner_id, ai.payment_term, ai.period_id,
                    u2.name AS uom_name,
                    (CASE
                    WHEN pt.id in (select product_tmpl_id from mrp_bom where type='phantom')
                        THEN '组合产品'
                        ELSE '独立产品'
                        END) AS product_type,
                        ai.section_id,
                    ...
                    count(ail.*) AS nbr,
                    ai.type, ai.state, pt.categ_id, pc.name AS categ_name, ai.date_due, ai.account_id, ail.account_id AS account_line_id,
                    ai.partner_bank_id,
                    SUM(CASE
    ...
    def _group_by(self):
        group_by_str = """
                GROUP BY ail.product_id, ai.date_invoice, ai.id,
                    ...
                    ai.fiscal_position, ai.user_id, ai.company_id, ai.type, ai.state, pt.categ_id,pc.name,
                    ai.date_due, ai.account_id, ail.account_id, ai.partner_bank_id, ai.residual,
                    ai.amount_total, ai.commercial_partner_id, partner.country_id, ail.price_unit,product_type
        """
        return group_by_str

    def init(self, cr):
        # self._table = qdoo_invoice_report
